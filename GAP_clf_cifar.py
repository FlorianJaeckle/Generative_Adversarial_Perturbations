...
import numpy as np
from OVAL.model_utils import load_cifar_1to1_exp
import pandas as pd
...
parser.add_argument('--fixed_eps', type=float, default=None, help='fixed l_inf magnitude of perturbation, not normalized')
...
#+
elif 'cifar' in opt.foolmodel:
        import torchvision.datasets as datasets
        import torchvision.transforms as transforms
        mean_arr = [0.485, 0.456, 0.406]
        stddev_arr = [0.225, 0.225, 0.225]
        normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                         std=[0.225, 0.225, 0.225])
        train_set = datasets.CIFAR10('../plnn-copy/cifardata/', train=True, download=False,
...
def train(epoch):
    ...
    for itr, (image, _) in enumerate(training_data_loader, 1):
        ...
        delta_im = normalize_and_scale(delta_im, 'train')  # uncommented in the original version
        ...
        # image[:,cii,:,:].min() is an estimation of the lower bound (almost always exact (if at least one image had originall a value of 0
        for cii in range(3):
...
def test():
    ...
    if opt.perturbation_type == 'universal':
        if opt.explicit_U:
            ...
            delta_im = normalize_and_scale(U_loaded, 'test')  #  uncommented in the original version
        else:
            delta_im = netG(noise_te)
            delta_im = normalize_and_scale(delta_im, 'test')  #  uncommented in the original version
    ...
    for itr, idx in enumerate(batch_ids):
        ...
        if opt.mode == 'test':
            file_ = opt.checkpoint + f'/netG_model_target_{opt.target}_final.pth'
            if os.path.isfile(file_):
                # print("=> loading checkpoint '{}'".format(file_))
                netG.load_state_dict(torch.load(file_, map_location=lambda storage, loc: storage))
                # print("=> loaded checkpoint '{}'".format(file_))
            else:
        ...
        if opt.perturbation_type == 'imdep':
            delta_im = netG(image)
            delta_im = normalize_and_scale(delta_im, 'test', eps_temp)  #  uncommented in the original version

        recons = torch.add(image.cuda(gpulist[0]), delta_im[0:image.size(0)].cuda(gpulist[0]))

        # no clamping need for our case
        # # do clamping per channel
        # for cii in range(3):
        #     recons[:,cii,:,:] = recons[:,cii,:,:].clone().clamp(image[:,cii,:,:].min(), image[:,cii,:,:].max())
        ...
        if itr % 50 == 1:
            ...
            post_l_inf_flo = (recons - image[0:recons.size(0)]).abs().max()
            # print("l inf flo", post_l_inf_flo)
...
def normalize_and_scale(delta_im, mode='train', eps=None):
    if opt.foolmodel == 'incv3':
    ...
    for i in range(bs):
        ...
        for ci in range(3):
            ...
            # TODO this is the line i need to change if i'm given a fixed new epsilon value
            mag_in_scaled_c = mag_in/(255.0*stddev_arr[ci])
            if eps:
                mag_in_scaled_c = eps
            elif opt.fixed_eps:
                mag_in_scaled_c = opt.fixed_eps
            else:
                mag_in_scaled_c = mag_in/(255.0*stddev_arr[ci])
...
def checkpoint_dict(epoch, final=False):
    netG.eval()
    global best_fooling
    if not os.path.exists(opt.expname):
        os.mkdir(opt.expname)
    ...
    net_g_model_out_path = opt.expname + f"/netG_model_target_{opt.target}_epoch_{epoch}.pth"
    if final:
        net_g_model_out_path = opt.expname + f"/netG_model_target_{opt.target}_final.pth"
    if opt.perturbation_type == 'universal':
        u_out_path = opt.expname + "/U_out/U_epoch_{}_".format(epoch) + task_label + "_{}.pth".format(test_fooling_history[epoch-1])
    if test_fooling_history[epoch-1] > best_fooling or final:
...
if opt.mode == 'train':
    ...
    checkpoint_dict(epoch, final=True)
    print_history()
elif opt.mode == 'test':
    print('Testing...')
    test()
